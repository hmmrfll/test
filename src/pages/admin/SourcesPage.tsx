...
export function SourcesPage() {
  const { sources, addSource, updateSourceStatus } = useContentStore();
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
                  ...
                  {filteredSources.map((source) => (
                    <TableRow key={source.id}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" className="text-xs text-muted" onClick={() => handleStatusToggle(source)}>
                          {source.status === 'active' ? 'Пауза' : 'Активировать'}
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))}
        ...
        <Card className="sticky top-8 h-fit shadow-inner dark:shadow-slate-950/40">
          <CardHeader>
